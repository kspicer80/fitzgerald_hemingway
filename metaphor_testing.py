...
hemingway_data = [(k.split("/")[2], v) for k, v in data.items() if k.startswith("data/hemingway/")]
fitzgerald_data = [(k.split("/")[2], v) for k, v in data.items() if k.startswith("data/fitzgerald/")]

hemingway_names, hemingway_counts = zip(*hemingway_data)
fitzgerald_names, fitzgerald_counts = zip(*fitzgerald_data)
print(hemingway_names, hemingway_counts)
...
rects1 = ax.bar(fitzgerald_names, fitzgerald_counts, bar_width, label='Fitzgerald')
rects2 = ax.bar(hemingway_names, hemingway_counts, bar_width, label='Hemingway')
...
plt.show()
plt.savefig('metaphor_counts.png')